...
function ExtractPatchCode(FilePath, VariableName, StopMarker, RepairSource = null) {
    ...
    if (StopIndex < 0) throw new Error(`server-v23 extraction failed: ${path.basename(FilePath)} ${StopMarker}`);
    ...
    if (typeof PatchCode !== "string" || !PatchCode.trim()) {
        throw new Error(`server-v23 extraction failed: ${VariableName}`);
    }
    return PatchCode;
}
...
function RepairV21Source(Source) {
    ...
    if (StartIndex < 0 || EndIndex < 0) {
        throw new Error("server-v23 repair failed: v21 listen patch block not found");
    }
...
if (!WrapperSource.includes(InjectionNeedle)) {
    throw new Error("server-v23 patch failed: v19 injection point");
}
